...
int main()
{
	...
	*/  //**********************************
	...
	char totallujing[40];
	int ttsswitch=0;
	in >> pass1 >> lujing >> loglujing >> totallujing>>ttsswitch;
	in.close();
	ifstream keyin(lujing, ios::in);
	if (!keyin)
	{
		//cout << "\a";
		//u盘未插入或者路径不正确，锁屏
	...
	jiemi2(pass2);
	string passmd5;
    passmd5= MD5(pass2).toString();
	
	//cout << pass1 << endl << pass2 << endl;
	if (passmd5.length()!=0&&passmd5==pass1)
	{
		ifstream logtotal(totallujing, ios::in);
		if (!logtotal)
		{
			cout << "\a" << endl;
			Sleep(1000);
		...
		else
		{
			...
			total++;
			logtotal.close();
			ofstream logtotal2(totallujing, ios::out);
			logtotal2 << total;
			logtotal2.close();
			SYSTEMTIME sys;                                    //获取系统时间
			GetLocalTime(&sys);
			ofstream log(loglujing, ios::app);
			log << "第" << total << "次：\t\t";
			log << "日期：";
			log << sys.wYear << "/" << sys.wMonth << "/" << sys.wDay << "\t\t";
			log << "时间：";
			log << setfill('0') << setw(2) << sys.wHour << ":" << setfill('0') << setw(2) << sys.wMinute << ":" << setfill('0') << setw(2) << sys.wSecond << endl;
			log.close();


			int tts = 1;
			//int year1,month1,day1,hour1, min1, sec1;
			ifstream ttsin("key.dll", ios::in);
			if (!ttsin)
			{
				//cout << "文件打开失败" << endl;
				cout << "\a";
			...
			{
				/*
				ttsin >>year1>>month1>>day1>> hour1 >> min1 >> sec1;
				struct tm time1;
				time1.tm_year = year1;
				time1.tm_mon = month1;
				time1.tm_mday = day1;
				...
				struct tm time2;
				GetLocalTime(&sysnow);
				time2.tm_year = sysnow.wYear;
				time2.tm_mon = sysnow.wMonth;
				time2.tm_mday = sysnow.wDay;
				time2.tm_hour = sysnow.wHour;
				time2.tm_min = sysnow.wMinute;
				time2.tm_sec = sysnow.wSecond;
				time_t time11, time22;
				time11 = mktime(&time1);
				time22 = mktime(&time2);
				diftime_sec = difftime(time22, time11);
				cout << diftime_sec << endl;
				*/
				time_t start, end;
				long int diftime;
				ttsin >> start;
				...
				diftime = end - start;
				fstream ttsout("key.dll", ios::out);
				ttsout << end;
				ttsout.close();
				//cout << diftime << endl;
				SYSTEMTIME sys;
				...
				if (ttsswitch == 1)
				{
					if (sys.wHour >= 6 && sys.wHour < 11)
					{
						if (diftime >= 86400)
						{
							int coverday = diftime / 86400;
							int coverhour = diftime%86400 / 3600;
							int covermin = diftime %86400 % 3600 / 60;
							int coversec = diftime %86400 % 3600 % 60;
							...
							if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
							{
								/*
								const char* s = "早上好。";
								const char* s1 = "距离您上次解锁已经过去";
								char* s12 = (char*)malloc(20);
								snprintf(s12, sizeof(s12), "%d", coverday);
								const char* s22 = "天";
								char* s2 = (char*)malloc(20);
								snprintf(s2, sizeof(s2), "%d", coverhour);
								const char* s3 = "小时";
								char* s4 = (char*)malloc(20);
								snprintf(s4, sizeof(s4), "%d", covermin);
								const char* s5 = "分钟";
								char* s6 = (char*)malloc(20);
								snprintf(s6, sizeof(s6), "%d", coversec);
								const char* s7 = "秒";
								*/
								char* szStr = (char*)malloc(200);
								sprintf(szStr, "早上好。距离您上次解锁已经过去%d天%d小时%d分钟%d秒", coverday,coverhour,covermin,coversec);
						...
						{
							int coverhour = diftime / 3600;
							int covermin = diftime % 3600 / 60;
							int coversec = diftime % 3600 % 60;
							ISpVoice* pVoice = NULL;                                            //创建一个ISpVoice的空指针
							if (FAILED(::CoInitialize(NULL)))                                     //初始化COM编程环境
							{
								cout << "初始化COM环境失败！" << endl;
							...
							if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
							{
								...
								sprintf(szStr, "%s%s%s%s%s%s%s%s", s, s1, s2, s3, s4, s5, s6, s7);
								WCHAR wszClassName[200];
								memset(wszClassName, 0, sizeof(wszClassName));
								MultiByteToWideChar(CP_ACP, 0, szStr, strlen(szStr) + 1, wszClassName,
									sizeof(wszClassName) / sizeof(wszClassName[0]));
						...
						else if (diftime < 3600 && diftime >= 60)
						{
							int covermin = diftime / 60;
							int coversec = diftime % 60;
							ISpVoice* pVoice = NULL;                                            //创建一个ISpVoice的空指针
							if (FAILED(::CoInitialize(NULL)))                                     //初始化COM编程环境
							{
								cout << "初始化COM环境失败！" << endl;
							...
							if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
							{
								...
								snprintf(s2, sizeof(s2), "%d", covermin);
								const char* s3 = "分钟";
								char* s4 = (char*)malloc(20);
								snprintf(s4, sizeof(s4), "%d", coversec);
								const char* s5 = "秒";
								char* szStr = (char*)malloc(200);
								//snprintf(szStr, sizeof(szStr), "%d", diftime);
								sprintf(szStr, "%s%s%s%s%s%s", s, s1, s2, s3, s4, s5);
								WCHAR wszClassName[200];
								memset(wszClassName, 0, sizeof(wszClassName));
								MultiByteToWideChar(CP_ACP, 0, szStr, strlen(szStr) + 1, wszClassName,
									sizeof(wszClassName) / sizeof(wszClassName[0]));
						...
						else
						{
							...
							if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
							{
								...
								const char* s1 = "距离您上次解锁已经过去";
								char* s2 = (char*)malloc(50);
								snprintf(s2, sizeof(s2), "%d", diftime);
								const char* s3 = "秒";
								char* szStr = (char*)malloc(200);
								//snprintf(szStr, sizeof(szStr), "%d", diftime);
								sprintf(szStr, "%s%s%s%s", s, s1, s2, s3);
								WCHAR wszClassName[200];
								memset(wszClassName, 0, sizeof(wszClassName));
								MultiByteToWideChar(CP_ACP, 0, szStr, strlen(szStr) + 1, wszClassName,
									sizeof(wszClassName) / sizeof(wszClassName[0]));
					...
					else if (sys.wHour >= 11 && sys.wHour <= 13)
					{
					    if (diftime >= 86400)
					{
						int coverday = diftime / 86400;
						...
						if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
						{
							...
							*/
							char* szStr = (char*)malloc(200);
							sprintf(szStr, "中午好。距离您上次解锁已经过去%d天%d小时%d分钟%d秒", coverday, coverhour, covermin, coversec);
							WCHAR wszClassName[200];
							memset(wszClassName, 0, sizeof(wszClassName));
							MultiByteToWideChar(CP_ACP, 0, szStr, strlen(szStr) + 1, wszClassName,
								sizeof(wszClassName) / sizeof(wszClassName[0]));
					...
					else if (sys.wHour > 13 && sys.wHour <= 18)
					{
				        if (diftime >= 86400)
					{
						int coverday = diftime / 86400;
						...
						if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
						{
							...
							*/
							char* szStr = (char*)malloc(200);
							sprintf(szStr, "下午好。距离您上次解锁已经过去%d天%d小时%d分钟%d秒", coverday, coverhour, covermin, coversec);
							WCHAR wszClassName[200];
							memset(wszClassName, 0, sizeof(wszClassName));
							MultiByteToWideChar(CP_ACP, 0, szStr, strlen(szStr) + 1, wszClassName,
								sizeof(wszClassName) / sizeof(wszClassName[0]));
					    ...
					    if (diftime >= 86400)
					{
						...
						if (SUCCEEDED(hr))                                                      //如果获取接口对象成功
						{
							...
							*/
							char* szStr = (char*)malloc(200);
							sprintf(szStr, "晚上好。距离您上次解锁已经过去%d天%d小时%d分钟%d秒", coverday, coverhour, covermin, coversec);
							WCHAR wszClassName[200];
							memset(wszClassName, 0, sizeof(wszClassName));
							MultiByteToWideChar(CP_ACP, 0, szStr, strlen(szStr) + 1, wszClassName,
								sizeof(wszClassName) / sizeof(wszClassName[0]));
